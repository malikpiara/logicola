 ...
 * takes the DSL's own "scoring off" level rather than a bare 0 — and with
...
export default function useQuizState(subSet: SubSet, initialMode?: QuizMode) {
  ...
  function handleNextQuestion() {
    ...
    // DSL `*m`: q := level, r := 8 — re-arm both registers for the new
  ...
  function onCheckAnswer(): 'correct' | 'miss' | undefined {
    ...
    if (isMulti) {
      ...
      if (wrongPicks.length === 0) {
        ...
        // DSL `ky:+$r` — awards 8 if clean, 0 if this problem was already

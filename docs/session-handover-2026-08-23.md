...
2. **A change of nothing shows nothing.** `chargeFor()` returns 0 for a run
   ...
   the set's original program has decayed to zero. The damage flicker already suppresses
...
meaningless for every published set, and per `the project notes` there are saved

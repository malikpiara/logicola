...
textbook and the original program** (option `a` is the correct one; a
...
**`useQuizState.tsx` is the other one to slow down on.** Per `the project notes`,
...
2. **`score_percentage` is meaningless and will corrupt tomorrow's
   ...
   numbers, today is the moment. Note `the project notes`'s rule: renaming a
...
- Dev server: the local launch config → `logicola-dev`, port 3100. Use the

...
| R | *undecided* | R / R2 / R3 | `#190B45` navy | `#1F0D92` indigo |
...
for the rest of that day — *and was itself retired the same day; **Set A is
A7**. See "The final cut" below.*

**L4 and A8 added** (2026-08-02), both built as in-between steps.

**L4** `#CFF6DD` / `#3F0167` / `#BD00AD` is the OKLCH midpoint of L and L2 on
*both* surface and ink — hue 157° between L's aqua-mint 174° and L2's pale
...
the palette. **A5 is not low-contrast.** Its ink clears **8.86:1**, *more* than
...
The general point: *pale* and *low-contrast* are different failures with
...
*Superseded: N5 was retired later the same day. Kept for the anchor
...
- **blue 264°** — VGA Blue, VGA Navy *and* his `#000077` link colour, all 264
...
the original *twice* and N2 sits between both).
...
the system's magenta arc, so the *chord* is the original's Lime/Magenta while
the *surface* supplies its Blue. Diverse surface, familiar chord — which is
the brief.
...
N5 alone confounds two questions: *which anchor* and *which chord*. N6 and N7
...
chord is the contribution** — a cool surface with a *green* ink is something
...
Note only **one** catalogue accent qualifies: `#24450A` is a *light* ink
...
The floor here is set by the ink, not by taste: `#24450A` is a *light* ink
...
236** — the band *between* the anchors, which is the territory the retired N2
...
is not a distinctiveness problem; distance to *other sets* is, and all four
...
— not a magenta. N3 is a *tonal aqua*: its surface and ink are 206° and 201°,
...
original `L` is a *different* colour scheme now sitting in RETIRED. Reusing
...
from G6 in OKLab — *closer to G6 than to its own sibling L* (.044). G6 is the
...
`N` now belong to *different*, retired schemes. Reusing a letter would put two
...
at **L .28–.32**, which is *ink* depth. The catalogue has surfaces at L .82–.93
...
Read it as "differ from the surface in hue *or* clearly in value". Gate 3 has

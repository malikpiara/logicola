...
| Caret  | `angle-down-solid`                        | FAQ accordion trigger (`components/ui/accordion.tsx`); the desktop nav trigger (`components/ui/navigation-menu.tsx`, 2026-09-04, replacing lucide's ChevronDown, the header's last smooth glyph); rotated -90° on the landing drill links | replaced the lucide ChevronDown; same size + rotate-on-open contract (NB Tailwind v4's `rotate-180` sets the standalone `rotate` property, not `transform`)              |
...
| Bars   | `bars`                                    | mobile header's exercises-menu trigger (`components/mobile/navbar.tsx`) | replaced a hand-drawn round-capped stroke hamburger (2026-09-04); rendered at 24px so the 2-unit bars are whole pixels |
| Copy   | `copy`                                    | the Keyboard page's copy buttons (`app/(site)/keyboard/page.tsx`) | replaced lucide's ClipboardCopy (2026-09-04) |

In-house, same grammar: the topic icons (`components/nav/topicIcon.tsx`, ∴ ⊃ ◇ O-ring bubble ? and, from 2026-09-04, a stepped ∀ for quantificational) are drawn as 4px rects rather than pasted — the library has no logic symbols. As of 2026-09-04 no `lucide-react` import remains and the dependency is removed; every glyph in the app is stepped.

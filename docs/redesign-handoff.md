...
## PORTED TO THE APP — 2026-08-07

The lab-to-app port happened. Everything decided (and every working
default) below now ships in the app; the lab remains the judgement bench.
What landed, and where:

- **The palette** — `components/quiz/quizColors.ts` (observed in
  `getQuizScreenColors`), six settled sets + the Q-by-id special case;
  Set R keeps its working orange surface with the settled ink/accent.
  Locked by tests: `quizColors.test.ts` asserts ink ≥ 7:1, accent ≥ 4.5:1
  on every surface, gate 3 (accent ≥ ~.10 OKLCH-L above ink, via
  `lib/contrast.ts`), and that ink/accent sit below 3:1 against each
  other — the fact that makes the 2px surface gap structural.
- **The silhouette generators** — `lib/pixel.ts`, byte-identical to the
  lab's (fixtures in `lib/__fixtures__/pixel-lab.json` were generated by
  running the lab's own JS; `pixel.test.ts` asserts string equality).
- **Options** — `components/option.tsx` + the `--qo-*` ladder in
  `app/globals.css`: sprite pills (R=24 list / R=12 grid), Tint
  treatment, band rings on the wrapper's `::before`, gem-clipped badge
  chips with ✕ (guessed only) / ✓ pixel marks, footprint-neutral
  selection. Focus = 2px band outside with the 2px surface gap; cursor =
  full-strength 2px on the silhouette (the 1.4.11 fix); both in the
  focus colour, never selection's.
- **Feedback** — `reserved` slot (`components/quiz/feedbackSlot.tsx`)
  above the options for BOTH layouts, ghost-sized per question; the
  structured hint (`components/quiz/hintBlock.tsx`): lamp in error tone,
  full name run-in, ink body, clause list with hanging numbers, `∴`
  chips, `Rule` emphasis. `Option` gained `hintParts` (threaded from
  `setR.data.ts`, whose aa clause now carries the restored `*must*`).
- **Scored-run-only** — `defaultModeForSet` in `quizMode.ts`; the start
  screen's level dial is the main control; count mode is dormant code
  plus the fallback for unscoreable sets. CTA copy resolved to
  "Start Quiz".
- **Chrome** — mono eyebrow header, "More than one answer can be
  right.", accent points label, pixel keycaps with verb labels, GUIDE
  ink-glass chip (R=12), notation chips at `spriteClip(0, 8)` everywhere
  (hints via `.qchip`, guide via `GUIDE_CODE_STYLE` in `wffGuide.tsx` —
  the small-instance padding lever applied, re-judge in KaTeX), prompt
  in cqw under a `.qcontainer` size container, the option grid on a
  container query (640px placeholder kept).
- **Mobile** — in-card sticky header row (bare ✕ linking home ·
  pixelised bar, sprite caps R=4/u=2, fill quantised via `round()` ·
  44×44 icon-only Guide chip that opens the sheet's guide snap);
  `overflow-clip` on the card (the sticky trap); ExerciseNavbar's white
  ✕ bar deleted; sheet CTA full-width.
- **Gem CTAs** — `components/quiz/gemButton.tsx` on start / footer /
  sheet / end screens. NB a clipped button paints no outline or ring, so
  focus is a band on the unclipped wrapper — the same reason the Guide
  chip's focus is a fill flip.
- **The pattern (2026-08-07, second pass; DECIDED by Malik on the third)**
  — `lib/patterns.ts` + `components/quiz/patternLayer.tsx`:
  **Camo · classic dresses the easy sets, Camo · giant the hard ones**
  (Malik, 2026-08-07 — "hard" is the catalogue's own word: subsets whose
  slug/title says Hard; unpaired subsets like modal Quantified and Set R
  count as easy until judged otherwise, flip in
  `patternKindForSubSet`). Quilt · pixel stays ported and
  fixture-locked as the dormant alternative. Placement per the decided
  contract — panel on start (and end) screens, the 112px footer band on
  desktop question screens (card reserves 152px), CLEAN on mobile
  question screens, 0.6 scale under 640px. Deterministic per set
  (seed = set name). The watercolor wash and its tuner are deleted.
  One deliberate deviation, argued in the code: the panel hole centres
  at 50% with a 560px cap on both breakpoints — the lab's 44%/480px was
  tuned around a filler hack (now removed) and never modelled the
  always-visible level dial, which it ran straight through. Generator
  parity with the lab is fixture-locked (`lib/patterns.test.ts`,
  including both camo variants).
- **Colour drift caught by Malik (2026-08-08), both corrected:** (1) a
  first port "refreshed" the accent pool to the shipped surfaces —
  exactly the incidental rewrite open item 4 warns against, and it
  recoloured every pattern render (Set A's camo accent turned mint; the
  judged render is lime). The pool is now the lab's `SET_SURFACES` +
  magenta VERBATIM, stale entries included; refreshing it stays open
  item 4, Malik's call in the lab. (2) Set R shipped on `#F2CDA6` — the
  RETIRED peach, which is not on the R dial at all. The working surface
  is now `R = #FFD199` (what the lab's R renders were judged on), with
  R2/R3 still live alternatives. Both are now test-locked:
  `quizColors.test.ts` accepts only dial candidates for R's surface,
  and `patterns.test.ts` pins the pool literally.
- **Reference guides (2026-08-07, third pass)** — `wffGuide.tsx` now
  renders the lab panel's information hierarchy (`SET_GUIDES` +
  `.qguide-*`, ported to a `.wff-guide` scope in globals.css): 15px/600
  headings, 13px body, chip rows for notation, dense tables with mono
  small-caps headers, hanging-numeral rule lists. Set R's eighteen
  fallacies read as the SAME structure as the hints — code chip,
  weighted name, ink gloss, numbered clause list with the "or" tails
  dropped, `*must*` and `∴` through the shared chip/emphasis machinery —
  replacing the shadcn three-column table Malik called a regression.
  **The panel's surface colour is now DECIDED (Malik, 2026-08-08): the
  sheet stays WHITE** — a set-surface panel was tried and rejected
  ("moved the focus from the quiz"; the quiz is the work, the panel is
  reference). The guide supplies neutral tokens to the shared
  chip/clause/emphasis machinery; the tokens stay var-driven, so the
  call is cheap to revisit if the lab ever argues otherwise. The
  sheet's chrome is branded instead (2026-08-08): stair-stepped left
  corners at R=12 with a 2px edge rule that follows the stairs (two
  stacked `sheetLeftClip` layers — a border loses its stroke on
  stairs), the sprite ✕ chip, the REFERENCE mono eyebrow, and a
  sprite-capped pixel resize grip.
- **Decision 7 (notation chips: ink vs accent) — RESOLVED, split by
  ground (Malik + arithmetic, 2026-08-08).** Colour for hierarchy, not
  background: in the REFERENCE the notation is what you scan for, so
  chips take the set's accent (text in the accent, plate at accent-10%
  on the white sheet — every accent clears 4.5:1 there), the typeable
  code column (R's `aa`…`sm`, Q's flaw numbers) goes full-strength
  accent, and the `Rule` emphasis mark gets its real accent back. The
  CARD's chips STAY INK: the accents sit at ~4.6:1 on their bare
  surfaces (gate 1 is tight by design), so accent text has no AA
  headroom over any darkened plate — measured 3.72–4.02:1 at a 12%
  tint. Same object, adaptive ink per ground, like the CTA. Only
  `--quiz-accent` rides into the sheets (never the surface); gates in
  `quizColors.test.ts` lock both halves, including a canary that fires
  if a future palette ever gives the card accents headroom.
- **Mobile is full-bleed (2026-08-07, third pass)** — below `lg` the
  quiz page hides the site navbar and drops its padding; card, start
  and end screens lose their rounding and fill the viewport. The white
  frame in Malik's screenshot was the page chrome showing through.
- **Four user reports, 2026-08-08, all landed:** (1) the once-per-question
  penalty memory was CHECKED AGAINST THE ORIGINAL original program (per the fidelity
  rule): Set R does charge once then disarm — and the app already
  mirrors it (hook-level tests now lock R/A/Q's three laws; Set A
  genuinely halves per miss — `k:-t` then `C:tt/2` in the wrong-answer
  branch — and Q is flat). (2) The progress bar read raw
  `score/TARGET`: a negative score (by design — no floor) emitted a
  negative width, which the browser DROPS, leaving the fill at
  auto/full — the "not updating" report. Both bars now read the clamped
  `progress()`. (3) `Check Answer` → `Next Question` shifted layout:
  the ultra-expanded face wrapped one label and not the other inside
  the fixed container — GemButton is `whitespace-nowrap` and the
  desktop containers widened to `w-52`; both labels now occupy an
  identical box. (4) The mobile CONTROLS sheet follows the set's
  colours (the lab's mobile-footer spec: surface fill, 2px ink-12% top
  rule, ink grabber, adaptive-ink CTA), all three `--quiz-*` vars ride
  in so the expanded guide inks itself, and the card's sheet clearance
  became padding instead of margin so no white page-strip shows. The
  white surface here was the CONTROLS sheet; the desktop REFERENCE
  pane stays white by the separate decision above.
- **Three working-value changes (Malik, 2026-08-08 evening):** (1) the
  pattern scatter RESHUFFLES — each mount rolls a fresh seed (the lab's
  Reshuffle made ambient; the deterministic per-set seed is gone).
  (2) **Set R wears S1** — surface `#E4BDF7` verbatim; ink and accent
  keep S1's hues at gate-clearing values (`#751100` rust at 7.04:1,
  `#824616` orange at 4.55:1 / gate-3 +0.096) because verbatim S1
  fails hard gates (rust ink 3.84:1 on the lilac; accent 0.10 BELOW
  the ink). The lab's S1 entry is untouched; Malik expects to change
  R again. (3) **Option chips wear the puffy diamond** (decision 8
  working value) as a percentage polygon of the lab's 32px literal —
  CARRIED FOR REVIEW on Q and R, whose 28px compact badges put the
  eighths on 3.5px steps, off the 4px grid.
- **A trap for the record:** the lab's `ring` state class is
  **Tailwind v4's `ring` utility** in the app — the collision painted a
  1px currentColor rectangle around every ringed option wrapper. The
  state class is `is-ringed` now. When porting lab class names, check
  them against Tailwind's utility namespace first.

**Still open after the port** (unchanged decisions, now with working
values live in the app): Set R's surface (and whether R stays orange),
base promotion (A7/L4/N10), answer treatment beyond Tint, Run-in vs
Heading, error tone (rose-mix ships), option chip square vs diamond,
R-guide pedagogy (decision 10/13), the stimulation-ceiling doc mismatch
(0.45–0.65 vs 0.45–0.58 — palettes are picked, so nothing shipped hangs
on it), `forced-colors` / `prefers-contrast`, the end screen's treatment
pass, and the guide panel's surface colour (the sheet stays white for
now — the lab's surface-coloured panel spec awaits the deferred
sheet/rail redesign).

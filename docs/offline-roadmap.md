...
3. **RSC navigations miss the cache.** `caches.match(event.request)` is
   called without `ignoreSearch`, so a client-side navigation requesting
   `/…/quiz?_rsc=abc` never matches the precached `/…/quiz`. Only hard
   page loads work offline today.

4. **Total offline payload is ~1.35 MB** — 1.2 MB shell plus 139 KB for
   all six generators. Per-quiz storage management is UI for a problem
   that does not exist at this size. This is the main argument for
   shipping a status badge rather than a toggle.
